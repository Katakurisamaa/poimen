...
type Receipt = { name: string; status: QuickAttendanceStatus; service?: string; previous: Change };

const REASONS = ["Maladie", "Travail", "Voyage", "Famille"];
const SERVICES = [
  { id: "culte_1", label: "Culte 1", short: "C1" },
  { id: "culte_2", label: "Culte 2", short: "C2" },
  { id: "culte_en_ligne", label: "En ligne", short: "Ligne" },
];
...
export default function FastCheckIn({ activity, date, members, onClose, onChange }: FastCheckInProps) {
  ...
  const activeMembersList = useMemo(() => members.filter(m => !m.archived), [members]);

  const rows = useMemo<Entry[]>(() => activeMembersList.map(member => {
  ...
  }).sort((a, b) => label(a.member).localeCompare(label(b.member), "fr", { sensitivity: "base" })), [activeMembersList, activity.id, date, isCulte]);
  ...
  const pointed = activeMembersList.length - counts.unpointed;
  const percent = activeMembersList.length ? Math.round(pointed / activeMembersList.length * 100) : 0;
  const complete = activeMembersList.length > 0 && counts.unpointed === 0;
  const filterCounts = { unpointed: counts.unpointed, present: counts.present, absent: counts.justified + counts.unjustified, all: activeMembersList.length };
  ...
  const save = async (change: Change, undo = false): Promise<boolean> => {
    ...
    try {
      ...
      return false;
    } finally { busyRef.current = false; setSavingId(null); setSavingService(null); }
  };
  ...
  return <dialog ref={dialogRef} className={`${styles.dialog} ${styles.surface}`} aria-labelledby="fast-checkin-title" onCancel={event => { event.preventDefault(); if (!busyRef.current) onClose(); }}>
    ...
    <div className={styles.workspace}>
      <aside className={styles.context} aria-label="Séance et progression">
        ...
        {isCulte && counts.present > 0 && (
          <div className={styles.culteSummary}>
            <span className={styles.eyebrow}>RÉPARTITION CULTES</span>
            <div className={styles.culteSummaryBadges}>
              {SERVICES.map(srv => {
                const c = rows.filter(r => r.status === "present" && r.service === srv.id).length;
                return (
                  <div key={srv.id} className={styles.culteBadge}>
                    <span>{srv.label}</span>
                    <strong>{c}</strong>
                  </div>
                );
        ...
        <div className={styles.progressBlock}>
          ...
          <div className={styles.progressTrack} role="progressbar" aria-label="Membres pointés" aria-valuenow={pointed} aria-valuemin={0} aria-valuemax={activeMembersList.length || 1}><span style={{ width: `${percent}%` }} /></div>
          <p>{pointed} sur {activeMembersList.length} {activeMembersList.length > 1 ? "membres pointés" : "membre pointé"}<span>{counts.unpointed} à pointer</span></p>
        </div>
      ...
      <section className={styles.workArea} aria-label={summary ? "Bilan du pointage" : "Liste des membres"}>
        {summary ? <div className={styles.summary}>
          ...
          <div className={styles.summaryHero}><span className={styles.summaryIcon}>{complete ? <CheckCheck size={32} /> : <ClipboardCheck size={32} />}</span><span className={styles.eyebrow}>BILAN DE LA SÉANCE</span><h2>{complete ? "Tout le monde est pointé." : activeMembersList.length ? "Un dernier coup d’œil." : "Aucun membre à pointer."}</h2><p>{complete ? "Les présences et les absences sont enregistrées. Vous pouvez quitter sereinement." : activeMembersList.length ? `${counts.unpointed} ${counts.unpointed > 1 ? "membres restent" : "membre reste"} à pointer. Vous pouvez continuer maintenant ou reprendre plus tard.` : "Les membres de votre groupe apparaîtront ici."}</p></div>
          ...
          <div className={styles.listTools}>
            <div className={styles.listHeading}>
              <div>
                <h2>Qui est présent ?</h2>
                <p>{isCulte ? "Un clic sur le culte pour pointer. Un motif pour une absence." : "Un clic pour une présence. Un motif pour une absence."}</p>
              </div>
              <span className={styles.total}><Users size={15} />{activeMembersList.length}</span>
            </div>
          ...
          <div ref={scrollRef} className={styles.memberList}>
            ...
            {visibleRows.map(entry => <article key={entry.member.id} className={styles.member} aria-label={label(entry.member)} aria-busy={savingId === entry.member.id}>
              ...
              <div className={styles.memberActions}>{entry.status === "unpointed" ? <>
                {isCulte ? (
                  <div className={styles.culteGroup} role="group" aria-label={`Pointer ${label(entry.member)}`}>
                    {SERVICES.map(srv => {
                      const isThisSaving = savingId === entry.member.id && savingService === srv.id;
                      const isOtherSaving = savingId === entry.member.id && savingService !== srv.id;
                      return (
                        <button
                          key={srv.id}
                          type="button"
                          className={styles.culteBtn}
                          disabled={saving}
                          data-active={isThisSaving ? "true" : undefined}
                          data-dimmed={isOtherSaving ? "true" : undefined}
                          title={`Pointer ${label(entry.member)} au ${srv.label}`}
                          aria-label={`Pointer ${label(entry.member)} présent au ${srv.label}`}
                          onClick={() => void save({ memberId: entry.member.id, status: "present", service: srv.id })}
                        >
                          {isThisSaving ? <LoaderCircle className={styles.spin} size={13} /> : <Check size={13} />}
                          <span className={styles.culteBtnFull}>{srv.label}</span>
                          <span className={styles.culteBtnShort}>{srv.short}</span>
                        </button>
                      );
                    })}
                  </div>
                ) : (
                  <button type="button" className={styles.presentButton} disabled={saving} aria-label={`Marquer ${label(entry.member)} présent`} onClick={() => void save({ memberId: entry.member.id, status: "present" })}>
                    {savingId === entry.member.id ? <LoaderCircle className={styles.spin} size={17} /> : <Check size={17} />}
                    <span>Présent</span>
                  </button>
                )}
    ...
    <footer className={styles.footer}>
      {failure && !editor ? <div className={styles.footerError} role="alert"><span>{failure.message}</span><button type="button" disabled={saving} onClick={() => void save(failure.change, failure.undo)}>Réessayer</button></div> : <div className={styles.saveState}>
        ...
        <div role="status" aria-live="polite"><strong>{saving ? "Enregistrement en cours…" : receipt ? `${receipt.name} · ${STATUS_LABELS[receipt.status]}${receipt.status === "present" && receipt.service ? ` (${SERVICES.find(s => s.id === receipt.service)?.label || receipt.service})` : ""}` : notice || "Enregistrement automatique"}</strong><span>{saving ? "Un instant, votre statut est en cours de sauvegarde." : receipt ? "Le statut a bien été enregistré." : "Vous pouvez fermer et reprendre à tout moment."}</span></div>
    ...
    {editor && <MemberEditor entry={editor.entry} initialStatus={editor.status} isCulte={isCulte} defaultService={editor.entry.service || "culte_1"} saving={saving} error={failure?.message || ""} onSave={save} onClose={() => { if (!busyRef.current) { setEditor(null); setFailure(null); } }} />}
  </dialog>;
}
