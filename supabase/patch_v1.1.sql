-- =============================================
-- Poimén Patch v1.1 - Migration & Fixes
-- =============================================

-- 1. Update Profile Roles Constraint
-- This allows 'responsable de brebi' and 'second du berger'
ALTER TABLE profiles DROP CONSTRAINT IF EXISTS profiles_role_check;
ALTER TABLE profiles ADD CONSTRAINT profiles_role_check 
  CHECK (role IN ('super_admin','berger','coordonnateur','responsable de brebi','second du berger','conseiller'));
...
DROP POLICY IF EXISTS "super_admin_invites" ON invites;
DROP POLICY IF EXISTS "berger_invites" ON invites;
DROP POLICY IF EXISTS "resp_invites" ON invites;
...
CREATE POLICY "berger_invites" ON invites FOR ALL 
  USING (bergerie_id = get_user_bergerie() AND get_user_role() IN ('berger','coordonnateur', 'second du berger'));
CREATE POLICY "resp_invites" ON invites FOR ALL
  USING (bergerie_id = get_user_bergerie() AND get_user_role() = 'responsable de brebi');
