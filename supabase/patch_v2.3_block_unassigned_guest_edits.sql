-- =============================================================
-- Poimén Patch v2.3 - Verrouillage des invités non assignés
-- =============================================================

-- 1. TRIGGER FUNCTION FOR INVITE UPDATE
...
-- Association du trigger BEFORE UPDATE
...
-- 2. TRIGGER FUNCTION FOR INVITE DELETE
CREATE OR REPLACE FUNCTION public.check_unassigned_invite_deletion()
RETURNS TRIGGER AS $$
DECLARE
  v_role TEXT;
BEGIN
  -- Cette règle s'applique spécifiquement aux invités de l'intégration
  IF OLD.church_id IS NOT NULL AND OLD.bergerie_id IS NULL AND OLD.assigned_to IS NULL THEN
    v_role := public.get_user_role_from_profile();
    -- Autorise la suppression uniquement aux rôles d'administration ou de direction de l'intégration
    IF v_role IN ('super_admin', 'integration_responsable', 'integration_second') THEN
      RETURN OLD;
    ELSE
      RAISE EXCEPTION 'Les données d''un invité non assigné ne peuvent être supprimées que par un responsable.';
    END IF;
  END IF;
  RETURN OLD;
END;
...
-- Association du trigger BEFORE DELETE
DROP TRIGGER IF EXISTS tr_check_unassigned_invite_deletion ON public.invites;
CREATE TRIGGER tr_check_unassigned_invite_deletion
  BEFORE DELETE ON public.invites
  FOR EACH ROW EXECUTE FUNCTION public.check_unassigned_invite_deletion();
